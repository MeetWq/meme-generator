...
class dot:
    ...
    def move(self,step:int)->None:
        ...
        self.velocity = (self.velocity[0]+a*self.direction[0],self.velocity[1]+a*self.direction[1])
        self.positon = (self.positon[0]+round(self.velocity[0]),self.positon[1]+round(self.velocity[1]))
    ...
    def out_of_img(self,img:Image.Image)->bool:
        ...
        if (
            self.radius <= 0 or
            self.positon[0]+self.radius < 0 or
            self.positon[0]-self.radius > w or
            self.positon[1]+self.radius < 0 or
            self.positon[1]-self.radius > h 
        ):
            return True
        return False
        ...
        def make(imgs:list[BuildImage])->BuildImage:
            ...
            if i < 36: 
                ...
                for x in range(width):
                    for y in range(max(0,min(height,height-round(step*i))),height):
                        pixel = (x,y)
                        ...
                        if distance <= step * (i - 12):
                            ...
                            if random.random() <= 0.06:
                                direction = ((x - o[0])/distance, (y - o[1]*1.5)/distance)
