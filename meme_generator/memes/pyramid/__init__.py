...
def crop_to_triangles(image: Image.Image, left: bool) -> Image.Image:
    images = (
        [image.copy()]
        if image.format != "GIF"
        else [frame.copy() for frame in ImageSequence.Iterator(image)]
    )
    if len(images) >= 24:
...
def pyramid(images: list[BuildImage], texts, args):
    ...
    def make_pyramid(img1: BuildImage, img2: BuildImage) -> BuildImage:
        ...
        for i in range(12):
            ...
            image1 = BuildImage(triangles1[(12 + i) % len(triangles1)]).perspective(
                params1[i][0]
            )
            bg.paste(image1, params1[i][1])
            if i:
                image2 = BuildImage(triangles2[i % len(triangles2)]).perspective(
                    params2[i][0]
                )
