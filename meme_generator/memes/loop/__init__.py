    ...
    def maker(i: int) -> Maker:
        def make(img: BuildImage) -> BuildImage:
            ...
            if not args.horizontal:
                ...
                return extend_img.crop((0, int(height/30*i), width, int(height/30*i)+height))
            extend_img = BuildImage.new("RGBA", (width*2, height), "white")
            extend_img.paste(img, (0, 0))
            extend_img.paste(img, (width, 0))
            return extend_img.crop((int(width/30*i), 0, int(width/30*i)+width, height))

        return make

    return make_gif_or_combined_gif(images[0], maker, 30, 0.05, FrameAlignPolicy.extend_loop)
