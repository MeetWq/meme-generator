...
group.add_argument("--right", "/右", action="store_const", const="right", dest="direction")
group.add_argument("--center", "/中间", action="store_const", const="center", dest="direction")


class Model(MemeArgsModel):
    direction: Literal[
        "left", "right", "center"] = Field(
        "center", description=help
    )
...
def fight_with_sunuo(images: List[BuildImage], texts, args: Model):
    ...
    def make(img: BuildImage) -> BuildImage:
        img = img.convert("RGBA").square().resize((565, 1630), keep_ratio=True, direction=modes[args.direction])
